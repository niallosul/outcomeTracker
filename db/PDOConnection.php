...
/*
	public function addppreltn($provid, $patid){
      $stmt = $this->_dbh->prepare("call addppreltn ($provid, $patid, @reply)");
      $stmt->execute();
      $resarr = $this->_dbh->query("select @reply as repstring");
      while($row = $resarr->fetch(PDO::FETCH_OBJ)) {
		  $returnval[] = $row;
		}
	  //var_dump($returnval);
	  //echo($returnval[0]->repstring);
	  return ($returnval[0]->repstring);
	}
*/
 }
?>